...
const callback = () => {
   ...
   addCopyButton('Användarnamn', 'Användarnamn\',\'Användarnamn, users-konto.', 22);
   addCopyButton('GKH-id', 'GKH-id\',\'Obligatorisk, unik identifierare för personer, enheter, funktioner, uppdrag och organisationer i GKH.', 23);
   addCopyButton('E-post', 'E-post\',\'Personens e-postadress hämtad från GKH.', 24);
...
function addCopyButton(property, longProperty, logNum) {
   ...
   copyButton.onclick = () => {
      try {
         ...
         alert('Kopieringen misslyckades. Kontakta Team om problemet kvarstår.');
         console.error(error);
         return;
      }
...
function sendLog(buttonID, currentObject) {
   ...
   fetch('https://serverx.domain.se/api/wass/log', {
      method: 'POST',
      headers: {
